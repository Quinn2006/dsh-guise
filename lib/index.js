 ...
 * @param {string} filePath - absolute path of the persona file.
 * @param {'global' | 'local'} scope - the layer the file belongs to; the
 *   client half keys its preset-mode UI off `resolved.source ===
 *   '<scope>-preset'`, so the scope must be the real layer name.
 */
function describeFile(home, filePath, scope) {
...
function resolveFileAt(home, filePath, scope) {
  ...
  if (lines.length === 1) {
    const match = /^@preset:([\p{L}\p{N}_-]{1,40})$/u.exec(first)
    if (match !== null) {
...
export function apply(ctx, config) {
  const cfg = {
    ...
    usageEnabled: config?.usage?.enabled ?? true,
    usagePrices: {
      input: typeof config?.usage?.prices?.input === 'number' ? config.usage.prices.input : DEFAULT_PRICES.input,
      output: typeof config?.usage?.prices?.output === 'number' ? config.usage.prices.output : DEFAULT_PRICES.output,
    },
  }
  ...
  // --- usage accounting: observe every committed session event and add the
  // --- tokens of each assistant message that carries a usage object to its
  // --- session and workspace (root listeners are untagged, so scoped
  // --- `session/event` dispatches bubble up to this host composition).
  let disposeUsage = undefined
  const usageLastSeq = new Map()
  if (cfg.usageEnabled) {
    disposeUsage = ctx.on('session/event', (session, event) => {
      try {
        if (event === null || typeof event !== 'object' || event.type !== 'assistant/message') return
        if (!readUsageSwitch(home)) return
        const usage = event.data?.usage
        if (usage === undefined || usage === null) return
        const sessionId = typeof session?.id === 'string' ? session.id : undefined
        if (sessionId === undefined) return
        const lastSeq = usageLastSeq.get(sessionId) ?? -1
        if (event.seq <= lastSeq) return
        usageLastSeq.set(sessionId, event.seq)
        const cwd = session.header?.cwd
        if (typeof cwd !== 'string' || cwd === '') return
        const normalized = normalizeUsage(usage)
        recordUsage(home, { cwd, sessionId, prompt: normalized.prompt, completion: normalized.completion, prices: cfg.usagePrices })
      } catch (error) {
        ctx.logger?.warn?.(`[dsh-guise] usage accounting failed: ${error instanceof Error ? error.message : String(error)}`)
      }
    })
  }
        ...
        const disposers = [
          ctx.webServer.register({
            ...
            handler: async (req, res) => {
              ...
              writeJson(res, 200, {
                ...
                global: describeFile(home, paths.global, 'global'),
                local: localPath === null ? { path: null, exists: false, raw: '', resolved: null } : describeFile(home, localPath, 'local'),
          ...
          ctx.webServer.register({
            ...
            handler: (req, res) => {
              ...
              const ledger = readUsageLedger(home)
              const withUsage = (workspace) => {
                const total = ledger.workspaces[workspace.path]?.total
                return {
                  id: workspace.id,
                  title: workspace.title,
                  path: workspace.path,
                  usage: total === undefined ? { tokens: 0, cost: 0 } : { tokens: total.tokens, cost: total.cost },
                }
              }
              let workspaces = []
              let source = 'storage'
              ...
              try {
                ...
                if (registry !== undefined) {
                  workspaces = registry.list().map(withUsage)
                  source = 'registry'
                }
              } catch (cause) {
              ...
              if (workspaces.length === 0) {
                ...
                if (fromStorage !== null && fromStorage.length > 0) {
                  workspaces = fromStorage.map(withUsage)
                  source = 'storage'
                }
              }
          ...
          ctx.webServer.register({
            ...
            handler: (req, res) => {
              ...
              const workspaces = Object.entries(data.workspaces).map(([cwd, workspace]) => ({
                cwd,
                tokens: workspace.total.tokens,
                cost: workspace.total.cost,
                sessions: Object.entries(workspace.sessions).map(([id, session]) => ({
                  id,
              ...
              for (const workspace of workspaces) {
                for (const session of workspace.sessions) {
                  bySession[session.id] = { cwd: workspace.cwd, tokens: session.tokens, cost: session.cost }
                }
              }
              writeJson(res, 200, {
                enabled: cfg.usageEnabled && readUsageSwitch(home),
                workspaces,
                bySession,
                path: usagePath(home),
                switchPath: usageSwitchPath(home),
              })
            },
          }),
          ctx.webServer.register({
            kind: 'exact',
            path: '/api/dsh-persona/usage/switch',
            handler: async (req, res) => {
              if (!isLoopbackRequest(req)) {
                writeJson(res, 403, { error: 'forbidden: loopback-only' })
                return
              ...
              try {
                const path = setUsageSwitch(home, body.enabled)
          ...
          ctx.webServer.register({
            kind: 'exact',
            path: '/api/dsh-persona/usage/reset',
            handler: async (req, res) => {
              if (!isLoopbackRequest(req)) {
                writeJson(res, 403, { error: 'forbidden: loopback-only' })
                return
              ...
              const body = await readJsonBody(req)
              try {
                resetUsage(home, typeof body?.cwd === 'string' ? body.cwd : undefined)
                writeJson(res, 200, { ok: true, cwd: typeof body?.cwd === 'string' ? body.cwd : null })
              } catch (error) {
                writeJson(res, 500, { error: error instanceof Error ? error.message : String(error) })
              }
            },
