...
/**
 * Validate and normalize a library preset id (40 chars max). Unicode-aware:
 * letters/digits of any script (including CJK) are preserved, everything
 * else — path separators, spaces, shell metacharacters — becomes `-`.
 * Preserving CJK keeps the id stable instead of silently renaming a preset
 * like `家猫` into a random `preset-xxxx` (which broke delete/edit and the
...
export function saveLibraryPreset(home, input) {
  ...
  const rawId = input.id !== undefined && String(input.id).trim() !== '' ? input.id : input.name
  // Editing an existing preset must reuse its real file (a CJK id like 家猫
  // would otherwise be renamed into a random slug, orphaning the old file).
  const existing = rawId !== undefined && rawId !== null && String(rawId).trim() !== '' ? findLibraryPreset(home, String(rawId)) : null
  const id = existing !== null ? existing.id : sanitizePresetId(rawId)
  const name = String(input.name ?? id).trim() || id
  // 名字行去重：面板编辑时 text 会带上原文件首行 `# 名称`，若再叠加
  // 一层会产生重复名字行；首行已是 `# ...` 时直接复用并同步为新名称。
  const bodyText = String(input.text).replace(/\r\n/g, '\n').trimEnd()
  const bodyLines = bodyText.split('\n')
  const body = bodyLines[0].trim().startsWith('# ')
    ? `# ${name}\n${bodyLines.slice(1).join('\n').trimEnd()}\n`
    : `# ${name}\n${bodyText}\n`
  const target = existing !== null ? existing.path : join(dir, `${id}.txt`)
...
export function deleteLibraryPreset(home, id) {
  // Match the real file first: ids that are not ASCII slugs (e.g. 家猫.txt
  // placed by hand) survive in listLibrary but never survive sanitization.
  const existing = findLibraryPreset(home, id)
  const target = existing !== null ? existing.path : join(personaPaths(home, '').libraryDir, `${sanitizePresetId(id)}.txt`)
  try {
    unlinkSync(target)
  } catch (error) {
