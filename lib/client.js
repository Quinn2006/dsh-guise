	...
	factory: (require) => {
		...
		function injectCss() {
			...
			style.textContent = [
				...
				"[data-dsh-guise-usage-banner]{display:flex;align-items:center;justify-content:flex-end;gap:6px;padding:3px 12px 1px;font-size:11.5px;color:#5f7192;user-select:none;overflow:hidden;white-space:nowrap;text-overflow:ellipsis}",
				"[data-dsh-guise-usage-banner] strong{color:#34d399;font-weight:600}",
			].join("\n")
		...
		function mountPanel() {
			...
			const refresh = async () => {
				...
				try {
					const [state, ws, balance, theme, history, usage] = await Promise.all([fetchState(cwd), fetchWorkspaces(), fetchBalanceInfo(), fetchTiredTheme(), fetchHistory(), fetchUsage()])
					...
					usageStatus.textContent = usageSwitch.checked ? `已统计 ${fmtTokens(usageTotals)} tokens · 约 ¥${usageCosts.toFixed(2)}` : "已暂停统计（历史数据保留）"
		...
		// ---------------- usage banner (conversation.input.dock slot) ----------------

		/**
		 * The composer banner: one line above the input card showing the CURRENT
		 * conversation's consumed cost and tokens. Mounted through the
		 * `conversation.input.dock` slot (session-scoped: `sessionId` arrives as
		 * a standard prop). Polls the host usage route so the numbers follow
		 * every model response. Renders null (no row at all) while the feature
		 * is switched off or the session has no recorded usage yet.
		 */
		function UsageBanner(props) {
			...
			react.useEffect(() => {
				...
				const tick = async () => {
					...
					} catch { /* banner is best-effort — never disturb the GUI */ }
				}
				tick()
				const timer = window.setInterval(tick, 15000)
				return () => { alive = false; window.clearInterval(timer) }
			}, [sessionId])
			if (usage === null || usage.enabled !== true) return null
			const entry = usage.bySession?.[sessionId]
			if (entry === undefined || entry === null) return null
			return react.createElement("div", { "data-dsh-guise-usage-banner": "" },
				react.createElement("span", null, "本对话消耗"),
				react.createElement("strong", null, "¥" + entry.cost.toFixed(2)),
				react.createElement("span", null, "· " + fmtTokens(entry.tokens) + " tokens"))
		}
			...
			try {
				...
				if (slots !== undefined && typeof slots.inject === "function") {
					disposers.push(slots.inject("conversation.input.dock", () => slots.register(
						{ name: "conversation.input.dock", id: "dsh-guise-usage", order: 40, label: "用量统计" },
						(props) => react.createElement(UsageBanner, props),
					)))
				}
			} catch (error) {
				console.warn("[dsh-guise] usage banner slot failed:", error)
			}
			ctx.effect(() => () => {
