	...
	factory: (require) => {
		...
		function injectCss() {
			...
			style.textContent = [
				...
				"[data-dsh-usage-bar]{position:fixed;top:0;left:50%;transform:translateX(-50%);z-index:9998;display:flex;align-items:center;gap:6px;padding:4px 12px;background:rgba(7,16,31,0.92);border:1px solid rgba(126,163,255,0.22);border-top:none;border-radius:0 0 10px 10px;color:#8fa3c4;font-size:11.5px;font-family:'Segoe UI','PingFang SC','Microsoft YaHei',system-ui,sans-serif;box-shadow:0 4px 14px rgba(1,4,12,0.35);pointer-events:none;user-select:none}",
		...
		/**
		 * 对话框上方用量浮条：屏幕顶部居中的小胶囊，显示当前对话（最近
		 * 活跃会话）已消耗的金额与 tokens。
		 *
		 * 设计原则（自研，非参考实现）：
		 *   - 只向 document.body 顶层 append 一个元素，绝不插入或修改 React
		 *     管理的任何节点（虚拟列表/滚动容器都不碰）；
		 *   - 不用 MutationObserver、不监听 DOM 变化——只有 15 秒心跳轮询；
		 *   - pointer-events:none，纯显示，永不遮挡点击；
		 *   - 任何异常静默降级，绝不影响页面。
		 * @returns {{ dispose: () => void }}
		 */
		function mountUsageBanner() {
			const bar = el("div", { "data-dsh-usage-bar": "", title: "本对话消耗（每 15 秒自动刷新）" })
