window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function mountPanel() {
			...
			const card = el("div", { "data-dsh-guise-card": "" },
				...
				el("div", { class: "pw-body" },
					...
					el("div", { class: "pw-section pw-balance-section" },
						...
						el("div", { class: "pw-actions" },
							el("button", { class: "pw-btn ghost pw-save-balance", type: "button", text: "保存阈值" }),
					...
					el("div", { class: "pw-section" },
						...
						el("div", { class: "pw-actions" },
							el("button", { class: "pw-btn pw-save-global", type: "button", text: "保存全局人设" }))),
					el("div", { class: "pw-section" },
						...
						el("div", { class: "pw-actions" },
							el("button", { class: "pw-btn pw-save-local", type: "button", text: "保存工作区人设" }))),
					el("div", { class: "pw-section" },
						...
						el("div", { style: "margin-top:10px" },
							...
							el("div", { class: "pw-actions" },
								el("button", { class: "pw-btn ghost pw-save-lib", type: "button", text: "存入人设库" }),
								el("button", { class: "pw-btn ghost pw-cancel-edit pw-hidden", type: "button", text: "取消编辑" })))),
					el("div", { class: "pw-section" },
			...
			// 按钮必须按独立 class 精确绑定——按数组索引绑定曾在「用量统计」
			// 区块插入后整体错位一格（点「保存阈值」实际执行了保存全局人设）。
			const saveBalanceBtn = card.querySelector(".pw-save-balance")
			const saveGlobalBtn = card.querySelector(".pw-save-global")
			const saveLocalBtn = card.querySelector(".pw-save-local")
			saveLibBtn = card.querySelector(".pw-save-lib")
			cancelEditBtn = card.querySelector(".pw-cancel-edit")
		...
		function mountUsageBanner() {
			...
			let currentWorkspaceTitle = undefined

			/** 侧边栏激活行签名：工作区/会话选中变化时签名变化。 */
			const currentSig = () => {
				let sig = ""
				for (const row of document.querySelectorAll('[class*="projectRow"], [class*="sessionRow"], [class*="folderActive"], [class*="workspaceRow"]')) {
					if (!(row instanceof HTMLElement)) continue
					const cls = typeof row.className === "string" ? row.className : ""
					if (!/(active|selected)/i.test(cls)) continue
					sig += cls + "|" + (row.textContent ?? "").replace(/\s+/g, " ").slice(0, 60) + "\n"
				}
				return sig
			}

			/** 工作区列表缓存 + 从激活行文本识别当前工作区。 */
			const loadWorkspaces = async () => {
				try {
					workspaces = (await readJson(await fetch("/api/dsh-persona/workspaces", { method: "GET" }))).workspaces ?? []
					const sig = currentSig().toLowerCase()
					for (const w of workspaces) {
						if (typeof w.title === "string" && w.title !== "" && sig.includes(w.title.toLowerCase())) {
							currentWorkspacePath = w.path
							currentWorkspaceTitle = w.title
							return
						}
					}
					currentWorkspacePath = undefined
					currentWorkspaceTitle = undefined
				} catch { /* 静默：沿用旧状态 */ }
			}
			...
			/** 侧边栏容器出现/重建时挂激活态观察。 */
			const attachSidebarWatch = () => {
				const col = sidebarRoot()
				if (col !== null && col !== sidebarEl) {
					...
					sidebarObserver.observe(col, { childList: true, subtree: true, attributes: true, attributeFilter: ["class"] })
					lastSig = currentSig()
				}
			}
			...
			const refresh = async () => {
				try {
					...
					// 统计信息 = 当前工作区的累计消耗（该工作区全部任务的总余额/总tokens）；
					// 识别不到当前工作区时退回全部工作区合计。
					let total = null
					if (currentWorkspacePath !== undefined) {
						const ws = (payload.workspaces ?? []).find((w) => w.cwd === currentWorkspacePath)
						if (ws !== undefined) total = ws
					}
					if (total === null) {
						total = (payload.workspaces ?? []).reduce(
							(sum, ws) => ({ tokens: sum.tokens + ws.tokens, cost: sum.cost + ws.cost }),
							{ tokens: 0, cost: 0 },
						)
					}
					bar.style.display = "flex"
					cost.textContent = `${currentWorkspaceTitle ?? "工作区"} ¥${total.cost.toFixed(2)}`
					tok.textContent = ` · ${fmtTokens(total.tokens)} tokens`
				} catch {
					bar.style.display = "none"
				}
			}
